...
func NewApi(r *gin.Engine, cfg *config.Config, store storage.StorageI, logger logger.LoggerI) {
	...
	// birinchi task
	r.POST("/exam", handler.ChangeProdsCount)

	/// ikkinchi task
	r.GET("/view/:time", handler.ViewDate)
	r.GET("/view", handler.ViewDate2)

	// uchinchi task
	r.POST("/promo", handler.CreatePromo)
	r.GET("/promo/:id", handler.GetByIdPromo)
	r.GET("/promo", handler.GetListPromo)
	r.DELETE("/promo/:id", handler.DeletePromo)

	// tortinchi task
	r.POST("/order_discount", handler.GetPromo)
